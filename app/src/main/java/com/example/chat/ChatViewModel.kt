...
class ChatViewModel @Inject constructor(@ApplicationContext val context: Context) : ViewModel() {
    ...
    fun mesajGonderme(channelID: String, mesajText: String?, image:String?=null){
        val message = Message(
        ...
        db.reference.child("messages").child(channelID).push().setValue(message)
            .addOnCompleteListener {
                if (it.isSuccessful) {
                    postNotificationToUsers(channelID, message.gonderenId, mesajText ?: "")
                }
            }
    }

    fun resimliMesajigonderme(uri: Uri, channelID: String){
        ...
        imageRef.putFile(uri)
            .continueWithTask {task->
                ...
                if(task.isSuccessful){
                    ...
                    mesajGonderme(channelID, mesajText = null, downloadUri.toString())
                }
    ...
    fun mesajBirimi(channelID: String){
    ...
    private fun postNotificationToUsers(
        ...
        val fcmUrl = "https://fcm.googleapis.com/v1/projects/chatktln/messages:send"
    ...
    private fun getAccessToken(): String {
        val inputStream = context.resources.openRawResource(R.raw.chatktln)
        val googleCreds = GoogleCredentials.fromStream(inputStream).createScoped(listOf("https://www.googleapis.com/auth/firebase.messaging"))
